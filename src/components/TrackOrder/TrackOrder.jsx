  ...
  useEffect(() => {
    async function getAuthToken() {
      try {
        const inputPayload = new URLSearchParams({
          ...
          client_id: import.meta.env.VITE_FEDEX_API_KEY,
          client_secret: import.meta.env.VITE_FEDEX_SECRET_KEY,
        });
        ...
        console.error(
          "Error authenticating with FedEx API:",
    ...
    async function getTrackingDetails() {
      try {
        ...
        const response = await axios.post(
          "/fedex-api/track/v1/trackingnumbers",
          inputPayload,
          { headers }
        );
        ...
        setOrderData(trackingDetails.reverse());
        setLoading(false);
      } catch (error) {
